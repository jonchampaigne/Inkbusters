...
final class Customer_Users
{
	...
	/**
	 * GETPREFS is a modified version of GetPrefs() from the User Class.
	 *
	 * This class is here because the User Class is not available to guest users, however the information is required to view and print Invoices
	 */
	...
	/**
	 * do the customer login process  [Note: admin logins are handled by strong_login() in class.user.php]
	 */
...
HEREDOC;
		...
		if (!$login = mysql_query($sql))
		{
			// send_email_alert($msg, $subject, $priority)
			send_email_alert('Failed customer login due to SQL syntax error.'
				. "\n"
				. "\n" . 'SQL: ' . $sql
				. "\n" . 'Error: ' . mysql_error()
				. "\n" . 'File: ' . __FILE__
				...
				,
				'Failed Customer Login, SQL Syntax Error'
				,
				'urgent'
			);
			return false;
		...
		if ( $login && mysql_num_rows($login)>=1 && $row=mysql_fetch_object($login) )
		{
			...
			{
				// send_email_alert($msg, $subject, $priority)
				send_email_alert('Failed customer login due to SQL syntax error when updating key.'
					. "\n"
					. "\n" . 'SQL: ' . $sql
					. "\n" . 'Error: ' . mysql_error()
					. "\n" . 'File: ' . __FILE__
					...
					. "\n" . 'Request URI: ' . $_SERVER['REQUEST_URI']
					,
					'Failed Customer Login, SQL Syntax Error Updating Key'
					,
					'urgent'
				);
				return false;
			...
			if ($update)
			{
				$this->key = $_SESSION['guest_user']->key = $key;
			}
...
HEREDOC;
			...
			if (!$info = mysql_query($sql))
			{
				// send_email_alert($msg, $subject, $priority)
				send_email_alert('Failed customer login due to SQL syntax error when getting user information.'
					. "\n"
					. "\n" . 'SQL: ' . $sql
					. "\n" . 'Error: ' . mysql_error()
					. "\n" . 'File: ' . __FILE__
					...
					. "\n" . 'Request URI: ' . $_SERVER['REQUEST_URI']
					,
					'Failed Customer Login, SQL Syntax Error Selecting User Info'
					,
					'urgent'
				);
				return false;
			...
			{
				// send_email_alert($msg, $subject, $priority)
				send_email_alert('Failed customer login due to an unhandled exception.'
					. "\n"
					. "\n" . 'SQL: ' . $sql
					. "\n" . 'Error: ' . mysql_error()
					. "\n" . 'File: ' . __FILE__
					...
					. "\n" . 'Request URI: ' . $_SERVER['REQUEST_URI']
					,
					'Failed Customer Login, Unhandled Exception'
					,
					'urgent'
				);
				return false;
		...
		{
			// send_email_alert($msg, $subject, $priority)
			send_email_alert('Failed customer login due to incorrect password.'
				. "\n"
				. "\n" . $this->db_customer . '.customer_user_accounts.user_account_id: ' . $this->id
				. "\n" . 'SQL: ' . $sql
				. "\n" . 'File: ' . __FILE__
				. "\n" . 'Line: ' . __LINE__
				. "\n" . 'Request URI: ' . $_SERVER['REQUEST_URI']
				,
				'Failed Customer Login, Incorrect Password'
				,
				'urgent'
			);
			return false;
	...
	/**
	 * verify customer as being logged in
	 *
