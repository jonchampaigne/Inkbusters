...
class Invoices_DB extends Customers_DB {
	...
	public function verify_ownership( $invoice_id=false, $user_id=false ){
		if( !$invoice_id || !$user_id ){
			...
			FROM bilyum_invoices.customers c
			INNER JOIN bilyum_invoices.customers_to_customers cc
			ON c.customer_id = cc.customer_id
			INNER JOIN bilyum_customeraccounts.customers_to_users cu
			ON cc.real_customer_id = cu.customer_id
			WHERE cu.user_id = '{$user_id}'	
			AND c.customer_id = '{$invoice_id}'
			LIMIT 1
HEREDOC;
			$check_ownership = mysql_query($sql) or die(mysql_error().'<br />Line: '.__LINE__);
